...
protected:
	...
	void OnWrite(const boost::system::error_code err, size_t bytes);      // д��Ϣ�ص�
	void OnRead();       // ����Ϣ�ص�
	void WriteHeartMsg();
	void PushMsg();
	void SendMsg(const std::string& send_msg, const std::size_t msg_len, const uint16_t msg_type);
	void StartTime();
	bool SpliteMsg(const char* data,const std::size_t len, std::size_t& msg_len);    // �ָ���Ϣ
	void HandleMsg(boost::shared_array<char> read_buffer, const boost::system::error_code & err, size_t bytes);    // ������Ϣ
...
private:
	...
	std::string rcv_msg_;
	uint16_t heart_interval_ = { 3 };   //����ʱ��
	std::atomic<uint16_t> heart_out_ = { 0 };		//�ۼ���������
};
