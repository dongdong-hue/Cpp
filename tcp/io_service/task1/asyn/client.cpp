...
public:
	Client() : /*sock_(__SERVICE), */started_(true)
	{}
...
private:
	void on_connect(const error_code & err) 
	{
		if (!err)
		{
			...
			// AsyncRead_ = boost::thread(boost::bind(&Client::Rev_msg, shared_from_this()));
			Rev_msg();
			StartTime();
		}   
		else  
	...
	void SendMsg(const std::string& send_msg, const uint16_t& msg_type)
	{
		...
		// std::cout << " ,tcp_msg->msg_len: " << tcp_msg->msg_len
		// 		  << " ,tcp_msg->msg_type: " << tcp_msg->msg_type << std::endl;
		memcpy(tcp_msg->msg_data, send_msg.data(), send_msg.length());
		// tcp_msg->msg_data[send_msg.length()] = '\n';
		// std::cout << "tcp_msg->msg_data: " << tcp_msg->msg_data;
	...
	void on_write(const error_code & err, std::size_t bytes) 
	{
		...
		{
			++send_num_;
			// std::cout << "send a msg, bytes: " << bytes << std::endl;
		}
	}
public:
	void do_write() 
